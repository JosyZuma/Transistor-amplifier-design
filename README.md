Single-Stage Transistor Amplifier (Common-Emitter, BC337)

A common-emitter amplifier built and simulated in Proteus, using a BC337 NPN transistor with a resistive voltage-divider bias network and capacitive input/output coupling.

Circuit values
Transistor: BC337 (NPN)
Supply: Vcc = 30V
Bias network: R2 = 56kΩ (Vcc→base), R3 = 33kΩ (base→ground)
Input: R1 = 2.2kΩ (series), C1 = 100nF (coupling)
Collector: R4 = 1.2kΩ
Emitter: R5 = 51Ω (unbypassed, sets gain), R6 = 1.2kΩ (bypassed by C2)
Output: C3 = 1nF (coupling), R7 = 10MΩ — a DC bias/pull-down after C3, not a real load

Bill of materials

<img width="938" height="726" alt="Screenshot 2026-07-27 000756" src="https://github.com/user-attachments/assets/2eae4c77-5195-4744-aaa2-dfdd4e30938f" />

Note: these parts were not individually purchased for this project — they were supplied as part of college laboratory stock for this academic assignment.

Expected gain

Av ≈ −R4 / R5 = −1200 / 51 ≈ −23.5

The output should be significantly larger than the input, and inverted.

DC bias point 
Base voltage: Vb ≈ Vcc × R3/(R2+R3) = 30 × 33k/89k ≈ 11.1V
Emitter voltage: Ve ≈ Vb − 0.7V ≈ 10.4V
Emitter current: Ie ≈ Ve / (R5+R6) = 10.4V / 1.251kΩ ≈ 8.3mA
Collector voltage: Vc ≈ Vcc − Ic·R4 ≈ 30 − 10.0 ≈ 20.0V
Vce ≈ Vc − Ve ≈ 9.6V

With Vcc = 30V, this puts Vce reasonably close to a usable operating point — not deep in saturation. 


What the simulation actually shows

<img width="421" height="271" alt="analog waveform" src="https://github.com/user-attachments/assets/9fc1d7fa-6a86-4bc7-b4f3-19d6f3e8b94c" />


The scope capture shows the output trace smaller than the input, which doesn't match the ~23x gain expected from the component values and a reasonable Q-point. Since the bias point checks out on paper, the mismatch points to something else worth checking rather than a bias design fault — candidates to verify: a bridged or cold solder joint (a few joints on the underside of the physical board look worth re-inspecting), the oscilloscope channel scaling/probe attenuation used when the capture was taken, or a loading effect from the test setup.

Build
Schematic and simulation done in Proteus

<img width="600" height="430" alt="Screenshot 2026-07-26 202551" src="https://github.com/user-attachments/assets/a3e56471-73bf-4a25-8575-b36e7da7bee0" />

<img width="596" height="426" alt="Screenshot 2026-07-26 202613" src="https://github.com/user-attachments/assets/5638a40b-c1d3-4dfe-bab1-72b2e1398faf" />

PCB layout

1. Board edge: Rectangular outline with chamfered top corners, drawn in CAD first to set the physical footprint before placing components

<img width="315" height="312" alt="Screenshot 2026-07-26 222424" src="https://github.com/user-attachments/assets/eef1ff52-a5aa-4519-ad7f-6fc5b1a8d466" />

2. Copper traces: Single-sided routing connecting the bias network, coupling caps, and connectors to the transistor's base, collector, and emitter pins

<img width="324" height="313" alt="Screenshot 2026-07-26 222953" src="https://github.com/user-attachments/assets/2e0003aa-15e2-43e2-9dd7-4632a35a2353" />

3. Silkscreen: Reference designators (R1–R7, C1–C3, Q2, T1–T3) plus title block — "Josy Zuma, Single Stage Amplifier PCB, Rev 1" with institution logo, for identification and revision tracking

<img width="316" height="320" alt="Screenshot 2026-07-26 223342" src="https://github.com/user-attachments/assets/6154a29c-10df-46fe-a39f-35f3ce2350ca" />

4. 3D visualiser: Used to check component footprints, connector orientation, and overall board layout before sending for fabrication

<img width="333" height="334" alt="Screenshot 2026-07-26 224043" src="https://github.com/user-attachments/assets/f06e58d6-9d0e-4042-8f42-8eb2bd0ec1a9" />

<img width="342" height="336" alt="Screenshot 2026-07-26 224031" src="https://github.com/user-attachments/assets/584b0203-cbce-4e26-bda7-6101a7c67ff5" />


Physical build

Bare board (no components): fabricated board showing the silkscreen outlines, copper traces, and drilled holes for R1–R7, C1–C3, Q2, and the T1–T3 connectors, before any soldering.

<img width="3024" height="4032" alt="IMG_1904" src="https://github.com/user-attachments/assets/3d6dd3e3-88cd-46ef-aece-1c119da44b4a" />

<img width="3024" height="4032" alt="IMG_1905" src="https://github.com/user-attachments/assets/227dee0a-672a-45cc-857e-373a71c2215d" />


Populated board (with components): resistors and capacitors soldered first (lower profile, easier to keep flat), transistor last to avoid heat damage from repeated reflow, connectors fitted with flying leads for power, input and output.

<img width="3024" height="2339" alt="IMG_1899" src="https://github.com/user-attachments/assets/740b688b-cf37-49c1-9163-b245c25d6021" />


<img width="3024" height="4032" alt="IMG_1900" src="https://github.com/user-attachments/assets/f6f6ccf8-5092-4c9f-ba48-91546f896275" />


Assembly & soldering

Underside view of the soldered board, showing component pin connections, trace routing, and the manual assembly work — resistor and capacitor leads soldered directly to the copper pads, with flying leads for T1–T3 routed off-board.

<img width="3024" height="3736" alt="IMG_1902" src="https://github.com/user-attachments/assets/8d4e5fbc-0bfe-4419-bfbd-78fef60d0406" />

<img width="3024" height="4032" alt="IMG_1903" src="https://github.com/user-attachments/assets/0872427c-de43-4194-a1b1-4a9275cfee5c" />



Design and Build Process

1.Circuit Schematics

Initial CAD schematic design and wiring layout

<img width="600" height="430" alt="Screenshot 2026-07-26 202551" src="https://github.com/user-attachments/assets/a3e56471-73bf-4a25-8575-b36e7da7bee0" />

<img width="596" height="426" alt="Screenshot 2026-07-26 202613" src="https://github.com/user-attachments/assets/5638a40b-c1d3-4dfe-bab1-72b2e1398faf" />


2.Output Waveform & Signal Analysis

Simulated input and output voltage waveforms captured using the built-in Proteus oscilloscope, demonstrating signal amplification and AC response.

<img width="421" height="271" alt="analog waveform" src="https://github.com/user-attachments/assets/9fc1d7fa-6a86-4bc7-b4f3-19d6f3e8b94c" />

3.PCB Layout & Design Workflow

3.1 Creating Board Edge

Defining the physical dimensions and boundaries of the printed circuit board layout in CAD.

<img width="315" height="312" alt="Screenshot 2026-07-26 222424" src="https://github.com/user-attachments/assets/eef1ff52-a5aa-4519-ad7f-6fc5b1a8d466" />


3.2 Placing Copper Traces

Routing the electrical connections between components (resistors R1–R7, capacitors C1–C3 and transistor Q1) ensuring proper clearance and signal flow.

<img width="324" height="313" alt="Screenshot 2026-07-26 222953" src="https://github.com/user-attachments/assets/2e0003aa-15e2-43e2-9dd7-4632a35a2353" />

3.3 Adding Silkscreen

Integrating component designators, terminal polarities, project title and institution branding onto the top silkscreen layer.

<img width="316" height="320" alt="Screenshot 2026-07-26 223342" src="https://github.com/user-attachments/assets/6154a29c-10df-46fe-a39f-35f3ce2350ca" />

3.4 3D Visualiser

Previewing the final assembled form factor and board layout using the 3D rendering tool prior to fabrication.
<img width="333" height="334" alt="Screenshot 2026-07-26 224043" src="https://github.com/user-attachments/assets/f06e58d6-9d0e-4042-8f42-8eb2bd0ec1a9" />

<img width="342" height="336" alt="Screenshot 2026-07-26 224031" src="https://github.com/user-attachments/assets/584b0203-cbce-4e26-bda7-6101a7c67ff5" />


4.Assembly Breakdown (Without Parts / Bare Board)

Unpopulated board layout and terminal block positioning
<img width="3024" height="4032" alt="IMG_1904" src="https://github.com/user-attachments/assets/3d6dd3e3-88cd-46ef-aece-1c119da44b4a" />



<img width="3024" height="4032" alt="IMG_1905" src="https://github.com/user-attachments/assets/227dee0a-672a-45cc-857e-373a71c2215d" />


5.Full Hardware Build

Custom 3D printed components and housing integration
<img width="3024" height="2339" alt="IMG_1899" src="https://github.com/user-attachments/assets/740b688b-cf37-49c1-9163-b245c25d6021" />


<img width="3024" height="4032" alt="IMG_1900" src="https://github.com/user-attachments/assets/f6f6ccf8-5092-4c9f-ba48-91546f896275" />

6.Assembly & Soldering

Underside view of the soldered PCB layout, showing component pin connections, trace routing and manual assembly
<img width="3024" height="3736" alt="IMG_1902" src="https://github.com/user-attachments/assets/8d4e5fbc-0bfe-4419-bfbd-78fef60d0406" />

<img width="3024" height="4032" alt="IMG_1903" src="https://github.com/user-attachments/assets/0872427c-de43-4194-a1b1-4a9275cfee5c" />


7.Bill of Materials (BOM)

Complete parts list detailing component references, quantities and specific values used in the circuit assembly
<img width="938" height="726" alt="Screenshot 2026-07-27 000756" src="https://github.com/user-attachments/assets/2eae4c77-5195-4744-aaa2-dfdd4e30938f" />

Note: These parts were not individually purchased for this project, as they were supplied as part of college laboratory stock for this academic assignment.
